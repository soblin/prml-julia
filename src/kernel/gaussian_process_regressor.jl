...
function log_likelihood(gp::GaussianProcessRegressor)
    return -0.5 * log(abs(det(gp._cov))) -
           0.5 * (transpose(gp._t) * gp._precision * gp._t) -
           0.5 * length(gp._t) * log(2.0 * pi)
end
...
function fitting(
    ...
    for i = 1:iter_max
        ...
        updates = [
            -tr(precision * grads[n, :, :]) +
            transpose(t) * precision * grads[n, :, :] * precision * t for n = 1:n_params
        ]
        for j = 1:iter_max
...
function predict(
    ...
    for n = 1:n_samples
        ...
        size(k_n) == (size(gp._X)[2], 1) ||
            error("size does not match: size(k_n) = $(size(k_n))")
