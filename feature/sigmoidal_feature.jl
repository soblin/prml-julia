mutable struct SigmoidalFeature
    _means::Array{Float64, 1}
    _coeff::Float64
    _n_kernels::Int64 # cache the number of kernels
    function SigmoidalFeature(means::Array{Float64, 1}, coeffs::Float64)
        new(means, coeffs, size(means)[1]);
    end
end

function _sigmoid(mean::Float64, coeff::Float64, x::Array{Float64, 1})
    return tanh.((x .- mean) .* (coeff * 0.5)) .* 0.5 .+ 0.5
end

function transform(feature::SigmoidalFeature, x::Array{Float64, 1})
